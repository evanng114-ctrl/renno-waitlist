...
function IntegrationCard({ item, delay }) {
  ...
  return (
    ...
    >
      ...
      <div>
        <p style={{ fontFamily: "'Archivo', sans-serif", fontWeight: 700, fontSize: 10, letterSpacing: '0.18em', textTransform: 'uppercase', color: 'var(--accent)', margin: '0 0 6px' }}>
          {item.label}
        </p>
  ...
  return (
    <section style={{ padding: 'clamp(56px, 6vw, 80px) 24px', background: '#F5F4F1' }}>
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        ...
        <div ref={header} className="reveal" style={{ textAlign: 'center', marginBottom: 48 }}>
          <p style={{ fontFamily: "'Archivo', sans-serif", fontWeight: 700, fontSize: 11, letterSpacing: '0.22em', textTransform: 'uppercase', color: 'var(--accent)', margin: '0 0 14px' }}>
            Integrations
          </p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 16 }}>
          {INTEGRATIONS.map((item, i) => (
            <IntegrationCard key={item.name} item={item} delay={(i % 4) + 1} />
          ))}
        </div>
